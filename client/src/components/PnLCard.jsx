    ...
    <div style={{ background: t.card, borderRadius: 8, padding: 20, border: `1px solid ${t.border}` }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 4 }}>
        ...
        <InfoModal title="Agent Performance">
          <div style={item}>
            <div style={{ fontSize: 12, color: t.sub, lineHeight: 1.5 }}><b style={{ color: t.accent }}>Revenue</b> - Total USD₮ received from verified on-chain payments. Only counts real transfers (wdk-settlement), not API requests without payment.</div>
          </div>
          <div style={item}>
            <div style={{ fontSize: 12, color: t.sub, lineHeight: 1.5 }}><b style={{ color: t.blue }}>Saved</b> - Total USD₮ the agent has autonomously transferred from Treasury to Savings. This is profit allocation, not an expense. The money is still owned by the agent in the Savings wallet.</div>
          </div>
          <div style={item}>
            <div style={{ fontSize: 12, color: t.sub, lineHeight: 1.5 }}><b style={{ color: t.accent }}>Total Assets</b> - Combined USD₮ across Treasury and Savings wallets. This is the agent's total holdings.</div>
          </div>
          <div style={{ fontSize: 12, color: t.muted, lineHeight: 1.5, marginTop: 12 }}>All data persists to disk and survives server restarts. All amounts in USD₮ (6 decimals).</div>
        </InfoModal>
      </div>
      ...
      <div style={{ borderTop: `1px solid ${t.border}`, paddingTop: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          ...
          <span style={{ color: t.accent, fontSize: 18, fontWeight: 'bold' }}>{totalAssets.toFixed(4)} USD₮</span>
        </div>
